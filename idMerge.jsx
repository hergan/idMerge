...
var exportFileName = defExportFileName(mergeType);

function vanillaMerge() {
  app.linkingPreferences.checkLinksAtOpen = false; // sets modified link dialog to false.
  var myTemplate = app.open(templateFile, false); // opens ID File headless (doesn't show active window)
  for (var i = 0; i < myTemplate.links.length; i++) {
    var templateLink = myTemplate.links[i];
  ...
  myTemplate.dataMergeProperties.selectDataSource(File(dataFile));
  myTemplate.dataMergeProperties.exportFile(saveName, "[High Quality Print]");
